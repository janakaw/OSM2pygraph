"""vector_calc.py: a collection of basic functions needed in processing geo-spatial data"""
...
""" The following code uses a vector v = (x,y,z) to locate a point in """
...
""" Calculate the line segment (Vb-Va) that is the shortest segment between
    two lines (v2-v1) and (v4-v3). Also calculates parameters mu_a and mu_b where
   	Va = v1 + mu_a (v2 - v1)
   	Vb = v3 + mu_b (v4 - v3)
    Return empty list if no unique solution exists (parallel or coincident lines). """
def line_line_intersection(v1,v2,v3,v4):
	...
	Va = [(v1[0] + mua*p21[0]), (v1[1] + mua*p21[1]), (v1[2] + mua*p21[2])]
	Vb = [(v3[0] + mua*p43[0]), (v3[1] + mua*p43[1]), (v3[2] + mua*p43[2])]

	return [Va, Vb, mua, mub]
...
def convert_geodetic_to_vector(lat,lon):
	pt_c = geodetic_to_ECEF(lat,lon)
	return vector(pt_c[0],pt_c[1],pt_c[2])

""" difference angle between v2v1 and v4v3 """
def get_diff_angle(v1, v2, v3, v4):
	return  abs(diff_angle((v1-v2),(v3-v4)))

""" magnitude of line segment v1v2 """
def get_mag(v1,v2):
	return mag(vector(v1[0], v1[1], v1[2])-vector(v2[0], v2[1], v2[2]))

""" Calculate perpendicular distance from a point v1 to the line segment (V2-V3)
    Function returns perpendicular distance d and flag acc.
    if v1,v2,v3 form an acute triangle acc = 0 otherwise acc = 1 """
def perpendicular_distance(v1, v2, v3):
	...
	if d == 0:
		return [acc, "inf"]	
	return [acc, mag(cross((v2-v3),(v1-v3)))/mg]
...
""" convert geodetic coordinates (latitude,longitude) into ECEF coordinates 
    altitude = 0 """ 
